...
export default function DeleteContact({ getnote }) {
  ...
  async function deleteContact() {
    const response = await fetch(`/api/notebook/delete-notebook/${getnote.id}`, {
      method: 'DELETE',
    })
    reset()
    refreshData()
              ...
              <div className="inline-block w-full max-w-lg p-6 my-8 overflow-hidden text-center align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                ...
                <div  className="flex flex-col w-full mt-5 space-y-5">
                  <div className="flex flex-col w-full space-y-1">
                    <span className="font-normal text-base text-[#333]">Are you sure you want to delete this note named <span className="font-bold">{ getnote.title }</span>?</span>
                  </div>
